...
class UserAnswerViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not Test.objects.filter(first_test=True).exists():
            return Response({"detail": "Нет доступных бесплатных тестов"}, status=status.HTTP_404_NOT_FOUND)

        if UserAnswer.objects.filter(user=user, test_content=test_content_id).exists():
            return Response({"detail": "Пользователь уже отвечал на этот контент теста"}, status=status.HTTP_400_BAD_REQUEST)

        test_content = TestContent.objects.get(id=test_content_id)
        if not test_content:
            return Response({"detail": "Контент теста не найден"}, status=status.HTTP_404_NOT_FOUND)

        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)

        statistics, created = UserStatistic.objects.get_or_create(user=user, test=test_content.test)
